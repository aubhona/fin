...
def get_data_for_xlx(
    uid, sdate, edate, min_sum, max_sum, cat=None
    ...
    profits = []
    if sdate:
        sdate, edate = datetime.date(
            datetime.strptime(sdate, "%Y-%m-%d")
        ), datetime.date(datetime.strptime(edate, "%Y-%m-%d"))
        for expense in Expenses.query.filter_by(user_id=uid).all():
            if (
                sdate
                <= datetime.date(datetime.strptime(expense.date, "%Y-%m-%d"))
                <= edate
            ) and (min_sum <= expense.price <= max_sum):
                expenses.append(expense)
        for profit in Profits.query.filter_by(user_id=uid).all():
            if (
                sdate
                <= datetime.date(datetime.strptime(profit.date, "%Y-%m-%d"))
                <= edate
            ) and (min_sum <= profit.price <= max_sum):
    ...
    if cat:
        if cat != set("all") and cat != "all":
            ...
            expenses = ex.copy()
    expenses.sort(
        key=lambda x: datetime.date(datetime.strptime(x.date, "%Y-%m-%d")), reverse=True
    )
    for expense in expenses:
...
def get_db_expences(uid, sdate, edate, min_sum, max_sum, cat=None):
    oper = []
    oper1 = []
    if sdate:
...
def count_exp(uid, cat):
    ...
    for expense in Expenses.query.filter_by(user_id=uid).all():
        if expense.cat == cat:
            s.add(
                datetime.date(datetime.strptime(expense.date, "%Y-%m-%d")).strftime(
                    "%m"
                )
            )
    return len(s)
